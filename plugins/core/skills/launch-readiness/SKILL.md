...
description: >-
  ...
  check whether a launch, release, or major change is actually ready to ship, or wants a
  canary/staged-rollout plan before exposing traffic — e.g. "출시 준비됐는지 점검해줘",
  "런치 체크리스트 만들어줘", "이 기능 배포해도 될지 검토해줘", "go/no-go 체크리스트",
  "canary 배포 계획 세워줘", "점진적 롤아웃 계획", "launch readiness review", "rollout plan
  before we ship this". The hard rule that makes this useful: every checklist item must
  resolve to a yes/no answer backed by a pointable artifact (a config, a dashboard URL, a
  runbook document, a test run) — "we have monitoring" with nothing to link is a FAIL, not
  a pass. Do NOT use it for routine deploys of already-established, unchanged behavior with
  no user-visible or load-profile change (the scope gate below exits those immediately), for
  post-incident retrospectives (a different exercise), or for doing the underlying
  implementation work itself (build the feature first; review readiness before flipping the
  switch on real traffic).
---
