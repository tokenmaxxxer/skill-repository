...
description: >-
  ...
  before any agent is spawned, where `merge-gates` decides what must be true before a branch lands.
  Use it before spawning more than one agent to BUILD something — e.g. "이 작업 병렬로 쪼개서
  시켜줘", "에이전트 여러 개로 나눠서 돌려", "50개 파일 마이그레이션 병렬로", "이거 동시에 시켜도 되나", "함대로 돌릴 수
  있게 나눠줘", "split this across agents", "fan this out in parallel", "can these run concurrently",
  "parallelize this migration/refactor", "how should I cut this up for agents". Trigger it whenever
  someone is about to fan out production work on a shared codebase, especially if the only reason is
  "there are many files." Do NOT use it for merge/landing questions (`merge-gates`), for research
  fan-out where agents read the world and share no artifact (`market-recon`, `tech-feasibility`
  carry that sweep discipline), for which model tier a piece runs on (`model-routing`), or for work
  small enough that briefing costs more than doing it (`model-routing` sets that bar).
---
...
routes that to `core:executor`), then come back.
