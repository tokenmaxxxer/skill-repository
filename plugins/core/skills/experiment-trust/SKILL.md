...
description: >-
  ...
  setup and the run's data quality BEFORE anyone interprets the result. Checks Sample Ratio Mismatch
  (SRM) via chi-square, platform A/A validation status, pre-committed design, and Twyman's-law
  skepticism on anomalous wins — so a broken assignment pipeline or an unvalidated platform never
  gets reported as a treatment effect. Use whenever someone is about to trust, report, or act on an
...
- **(c) NOT verified in our research**: sample-size / power-analysis specifics, and the peeking
  (early-stopping) type-I-error-inflation literature. The gates below still REQUIRE a pre-committed
  sample size and a no-peeking rule — but that is a procedural design choice consistent with the
  verified A/A false-positive semantics (a fixed nominal α presumes a fixed horizon), not a claim this
  skill's research checked. For the pre-registration discipline itself — metric, threshold, decision
...
Assemble the full report; do not compress any of the following away:

- Pre-committed design vs. what actually ran, with every deviation named.
- The SRM verdict, with its expected split, observed counts, and p-value stated side by side (Step 4).
- The A/A platform status: validated (with its false-positive rate), failed validation, or unvalidated (Step 2).
- The effect, with its confidence interval — **only if Step 4 passed**. An SRM-flagged run's report
  contains no effect estimate, no confidence interval, and no direction: only the SRM verdict and the
  pipeline-investigation status. The same holds for a platform in the "failed A/A validation" state.
- The base-rate reminder (medium grade, 2-1 — name the grade when citing): most product ideas do not produce a positive
  significant effect. Microsoft's own data shows roughly a 1/3–1/3–1/3 split (positive/flat/negative);
  Google reportedly ran ~12,000 randomized experiments in 2009 with only ~10% leading to business
  changes; at Amazon "half of the experiments failed to show improvement." A flat result is the norm,
  not a failure of execution — do not let this be read as license to keep re-testing until a positive
  result appears.
