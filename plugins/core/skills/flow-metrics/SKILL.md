...
description: >-
  ...
  reports honestly what the evidence does and does not support about WIP limits. Use this whenever
  the user wants to understand, measure, or diagnose work flow — e.g. "왜 이렇게 오래 걸리지", "리드타임
  계산해줘", "WIP 제한 걸어야 하나", "흐름 지표 분석해줘", "칸반 보드 진단", "why is this taking so long", "should we
  set a WIP limit", "calculate our lead time", "diagnose our kanban board", "will lowering WIP
  actually help". The skill's headline value is CORRECTING a widespread belief: in the one
  quantitative case study available, lower WIP correlates with shorter lead time and correlates with
  productivity in the direction opposite to the kanban claim — neither is a controlled result. Lead
  with that honesty, not with "lower your WIP". Do NOT use it when there is no per-item
  entry/exit timestamp data and no willingness to instrument it (the first action is instrumenting
  start/finish events, not computing a metric — say so plainly), when the question is about an
  individual's personal performance rather than a system's flow, or when the team has already
  decided to change a WIP limit and just wants execution/measurement help (route the before/after
  evaluation to `hypothesis-testing` instead of re-running this diagnostic).
---
