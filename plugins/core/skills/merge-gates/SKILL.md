...
**Gate**: no numeric risk threshold is written without the sentence "this is convention, not measurement" beside it. The only evidence-based route to a tier is to reproduce the safe-side classifier on your own history and measure its precision — if that clears 0.9 on your data, you have a validated pre-filter and this step can be revisited.
