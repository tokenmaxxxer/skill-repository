...
description: >-
  ...
  derive falsifiable hypotheses from it, PRE-REGISTER the metric, threshold, and decision rule
  before any data is collected, then let that registered rule — not post-hoc judgment — make the
  kill/pivot/persist call. Use this whenever someone wants to test whether an idea, feature, or
  direction actually works before sinking more into it — e.g. "이 아이디어 될지 검증해보자", "계속할지 접을지
  모르겠어", "피벗해야 하나", "이 기능 만들까 말까", "가설 세워서 테스트하자", "how do we know if this is working",
  "should we kill this or keep going", "let's set up an experiment for this", "define success
  criteria before we build this". Trigger it whenever a go/kill/pivot/persist decision is still
  open and the metric or threshold for judging it has not yet been written down. Do NOT use it
  once a test is already running with thresholds already fixed (just run it to the registered
  rule), for a decision already committed with no realistic option to stop (that's execution, not
  testing), for pure preference/creative-taste calls where no observable outcome could settle the
  question (escalate those via decision-brief instead), or for a single reversible implementation
  detail that doesn't warrant a registration form.
---
