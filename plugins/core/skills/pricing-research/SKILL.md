...
description: >-
  ...
  family by what the pricing decision actually needs to know, and reporting each method's numbers
  with the labels their evidence base actually supports. Use this whenever someone needs to set,
  test, or audit a price or price range for a defined product or feature bundle — e.g. "가격 얼마로
  정해야 할까", "지불의사 조사해줘", "컨조인트 설계해줘", "이 가격 조사 결과 믿어도 돼", "밴 웨스텐도프 가격 조사", "PSM 설계해줘",
  "how much should we charge", "willingness to pay research", "design a conjoint study", "should I
  trust this pricing research", "price sensitivity meter", "what price maximizes revenue". Trigger
  it whenever a team is about to pick a pricing-research method, design one, or hand off pricing
  numbers as if they answer a question the method didn't actually collect data for. Do NOT use it
  for "what does the market/competitors charge" (that is competitor research — route to
  `market-recon` instead), for pricing decisions with no enumerable product or feature bundle yet
  (too early — say so), or for general market-sizing/demand questions with no price variable at
  their center.
---
...
Never compress this into a bare price recommendation — the whole point of this skill is that the
number is traceable to a named method and family, a stated set of inputs it actually collected,
and gates that were checked before the number was trusted, not reverse-engineered to justify a
price someone already wanted.
