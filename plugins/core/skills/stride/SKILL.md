...
name: stride
description: >-
  A design-stage security threat analysis procedure (STRIDE threat modeling, per-element):
  ...
  sibling of the registry's fmea skill — same enumerate-and-disposition shape, applied to threats
  instead of failure modes. Use this when the user wants to find what could be attacked in a
  design before it ships — e.g. "이 설계 보안 위협 분석해줘", "위협 모델링 해줘", "STRIDE 분석", "이 아키텍처에서
  뭐가 뚫릴 수 있어", "threat model this design", "what can an attacker do to this system", "run a
  STRIDE analysis", "security review of this architecture before we build it". Do NOT use it for
  code-level vulnerability scanning of an already-written codebase (that's a static/dynamic
  analysis or code-review task, not this), for compliance or regulatory mapping (point to
  `compliance-scan`), for privacy-specific threats such as identifiability, linkability, or
  unawareness of data use (that's `LINDDUN`'s fit, not STRIDE's), or when there is no concrete
  design/architecture with enumerable elements and data flows to decompose yet.
---
...
- **Is this actually a request to scan existing code for vulnerabilities?** Per-element STRIDE analyzes a design on paper (or a diagram), not source code line by line. If the ask is "find the bugs in this codebase," that is static/dynamic analysis or `code-review`, not this skill.
...
For every element from Step 2, walk each category the table marks as applicable to that element's type. For each (element, category) cell, record either at least one concrete threat phrased as "an attacker could ..." or an explicit "none identified" note with a one-line reason.
...
**Gate:** the element × applicable-category matrix has no blank cells. Every cell is either a named threat or a reasoned "none identified" note. A cell left empty does not pass.
...
- The full list of cells marked "none identified" in Step 3 (so they are auditable later, if the design changes).
...
**Gate:** the report states all three counts, lists every "none identified" cell (not just a total), and includes the standing-caveats paragraph verbatim in substance — omitting the caveats is treating heuristic guidance as proven fact.
