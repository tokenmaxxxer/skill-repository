...
### Step 2a — Classify components by severity potential

Before enumerating failure modes for every component, classify each by the worst credible severity its failure could cause. This gate prevents the worksheet from expanding to N×M×(Effect+Cause+S+O+D) for components whose failure has trivial consequence.

Score every component from Step 2 on its worst-case severity potential:

| Level | Criterion | Analysis depth |
|---|---|---|
| **High** | Failure could cause safety/regulatory harm, data loss, or unrecoverable core-function outage; OR this component is a single point of failure for a critical path | Full enumeration per Step 3–7. Every S/O/D rating cites its band anchor text. Every failure mode gets full effect+cause+disposition. |
| **Medium** | Failure degrades user experience or non-critical function, but does not cause safety/regulatory harm or irrecoverable loss | Full enumeration, but band citations may be the band label only (e.g., "S=6 — Moderate"), not the full anchor description. |
| **Low** | Failure causes cosmetic or trivial annoyance; redundant component with automatic failover; failure of this component alone cannot propagate to user impact | Aggregate: "N components (list names) — failure modes are cosmetic/self-contained, no credible path to user harm." Full worksheet columns not required. |

**Gate:** every component has a written High/Medium/Low classification with a one-phrase rationale (e.g., "High — single point of failure in auth path"). Record as a compact table before proceeding. This classification is the audit record for why depth varied.

### Step 3 — Enumerate failure modes per element

For every element from Step 2, list at least one way it could fail to perform its intended function ("how could this fail to do its job"). Apply the depth rule from Step 2a: High and Medium elements get individually enumerated failure modes. Low elements are covered by their Step 2a aggregate note.

**Gate:** every High and Medium element has either (a) at least one failure mode listed, or (b) an explicit note "no credible failure mode identified" with a one-line reason. No High or Medium element may be left blank. Low elements are accounted for by the Step 2a classification.

### Step 4 — Effect and cause per failure mode

For each failure mode on High and Medium elements, fill in:
- **Effect** — what the user or downstream system actually experiences.
- **Cause** — why the failure mode would occur.

**Gate:** both cells are filled for every failure mode on High and Medium elements. A failure mode with an empty effect or cause is incomplete, not "obvious enough to skip." Low-element failure modes are covered by the Step 2a aggregate note.

### Step 5 — Rate S, O, D against anchors

Rate every failure mode on High and Medium elements on Severity, Occurrence, and Detection using the 1–10 scales below. Ratings must be made against the written anchor bands, not gut feel.
...
**Gate:** for High-element failure modes, every S/O/D rating cites the full band anchor text (e.g., "S=9 — band: safety issue with warning"), for all three scales. A bare number with no band citation does not pass for High. For Medium-element failure modes, the band label alone suffices (e.g., "S=6 — Moderate"). Low-element failure modes are not rated individually.
...
For every mitigation decided in Step 6 for High and Medium failure modes, record:
- **Owner** — a named person or role.
- **Expected re-rating** — the S/O/D the team expects *after* the action is done.

For Low-element failure modes, a single aggregate action row suffices: "N Low-severity failure modes — no individual actions required; monitor as a group" with an owner.

**Gate:** every High and Medium action row has an owner and a post-action S/O/D estimate filled in. Low elements have the aggregate action row with an owner. An action with no owner or no re-rating is not a completed action, it's an idea.
