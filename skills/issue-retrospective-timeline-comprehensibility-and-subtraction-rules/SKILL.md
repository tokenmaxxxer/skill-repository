...
description: >-
  Use when composing or reading a records-only cross-skill retrospective for a subject issue
...
skill's specific object (a *records-only* cross-skill retrospective, not a live
...
Use this skill when composing or reading a records-only cross-skill
...
such as mid-incident postmortem drafting or a single skill's own
implementation record:
- starting or continuing composition of the retrospective record from
  another skill's records (Timeline, Contributing factors, What we
...
1. Draft the Timeline section first, from the subject's other-skill
...
2. While drafting or reviewing Contributing factors, rewrite any
   sentence that names a person or skill's choice as the explanation so
...
6. Before finalizing, run the two subtraction passes: cut any section
   ...
   or another skill's record in favor of a citation (rule 8), and cut any
...
- 1.1 — When starting record composition, write the Timeline section from the subject's other-skill records first, before drafting any Contributing factors or What-we-learned lan…
- 1.2 — When a contributing-factor sentence is about to name a person or a skill's choice as the explanation, rewrite it to name the structural gap (process, tool, documentation,…
...
- 1.4 — When drafting Action items, give each one a named owner (a person or skill, never "the team") and phrase it as a checkable change, and mark whether it fixes this instance…
- 1.5 — When the record will be read by a session or reviewer unfamiliar with the subject issue's jargon (skill names, internal gate names, tool acronyms), define the term at its…
...
- 1.7 — When another skill's record is silent on a section this skill's contract requires reading (e.g., no Impact summary in an implementation record), record that silence itself…
- 1.8 — **REMOVAL**: When a draft record section restates background already established by the linked issue, proposal, or another skill's record (e.g., a "Context" section re-ex…
...
- 1.14 — When any sibling skill record for the subject is still at a non-terminal loop_state at the moment this skill reads it, say so plainly in the Timeline (or a dedicated note…
