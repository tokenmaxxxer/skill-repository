...
### Step 4a — Classify stories by audit priority

Before running the full 13-criteria QUS matrix on every story, classify each by audit priority so the procedure invests deep scrutiny where it matters and compresses where it doesn't. A cosmetic story that passed the template check doesn't need the same 13-cell treatment as a payment-flow story.

Score each story on two axes:

- **A: Impact of ambiguity** — does this story touch safety, regulatory compliance, payments, auth, data integrity, or core revenue path?
- **B: Template health** — did it fail the Step 4 template check? (A failing template is a signal the story may be poorly formed and needs deeper scrutiny.)

| Level | Rule | QUS depth |
|---|---|---|
| **High** | A is yes OR B is fail | Full 13-criteria pass/fail, one-line reason for every fail. |
| **Medium** | Neither A nor B, but user-facing functional behavior | Full 13-criteria pass/fail, but fail reasons compressed to category label only (e.g., "ambiguous term" without quoting the term). |
| **Low** | Neither A nor B, cosmetic/informational/trivial, template check passed | Template-only — record "QUS skipped: Low priority, template passed." No 13-criteria matrix needed. |

**Gate:** every story has a written High/Medium/Low classification with one-phrase rationale. Record as a compact table before proceeding. This classification is the audit record — a reviewer can see that Low stories were explicitly judged low-priority, not silently skipped.
...
**Output:** a violations table, story × criterion. For High stories: one row per story, every criterion marked pass/fail with a one-line reason for every fail. For Medium stories: pass/fail with category-only fail labels. For Low stories: a single row "N Low-priority stories — QUS skipped per Step 4a classification."

**Gate:** the table accounts for every story — High and Medium stories have all 13 criteria checked; Low stories have the explicit skip note. No story is left unaccounted.

### Step 6 — Report

Deliver four things, in this order:

1. **Classification table** from Step 4a (story × priority × rationale — the audit record for why QUS depth varied).
2. **Violations found** — pulled directly from the Step 3/4/5 tables.
3. **Rewrites proposed** — one rewritten version per failing High/Medium item, re-checked against the same gate it failed.
4. **Residual list** — what this audit does *not* establish (see the evidence-grade section below; always carry the document-quality-vs-development-outcome distinction into this list by name).
