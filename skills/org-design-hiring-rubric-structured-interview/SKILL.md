...
description: >-
  Use when designing or reviewing a hiring interview rubric, or judging whether
  an interview process is structured enough to trust its validity claim. Trigger
  on requests like "면접 루브릭 만들어줘", "design a structured interview", "behaviorally
  anchored rating scales for interviewers", "can we trust this interview's
  validity". Do NOT use for defining what a role's competencies are in the first
  place (use org-design-role-competency-definition) or for measuring an existing
